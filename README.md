...
* The `decode-signature` value tells the policy to decode the signature value as
  a base16 string, resulting in a byte array.
...
IEEE P1363-formatted signature (simply concatenating the (r,s) pair), you need to specify that in the policy configuration with the `format` property.
